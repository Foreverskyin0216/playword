...
export const getRecordings = (record: string | boolean) => {
  const path = typeof record === 'string' ? record : '.playword/recordings.json'
  if (!existsSync(path)) return false
  else return JSON.parse(readFileSync(path, 'utf-8')) as Recording[]
}
